...
if __name__ == "__main__":
    ...
    # sample a random joint config
    theta = torch.rand(1, 7, device=device)

    # base pose = identity
    pose = torch.from_numpy(np.identity(4, dtype=np.float32)).to(device)
    pose = pose.reshape(1, 4, 4).expand(theta.shape[0], -1, -1)

    # get posed mesh and export
    robot_mesh_list = panda.get_forward_robot_mesh(pose, theta)
    robot_mesh = np.sum(robot_mesh_list)  # merge all links

    os.makedirs("output_meshes", exist_ok=True)
    out_path = os.path.join("output_meshes", "whole_body_levelset_0.stl")
    trimesh.exchange.export.export_mesh(robot_mesh, out_path)
    print("Saved mesh to:", os.path.abspath(out_path))
