...
if __name__ == "__main__":
    ...
    try:
        for i in range(int(duration / stepsize)):
            ...
            height_diff = abs(end_pos[2] - target_pos[2])
            ...
            if grad_gamma is not None and sca_debug:
                ...
                try:
                    ...
                    prob = None
                while prob.status != cp.OPTIMAL and eps_sc > 1e-2:
                    eps_sc = eps_sc - 0.01
                    rhs_sc = eps_sc - c_const_sc + g_eff_sc @ M_inv @ tau_id
        ...
        # Ensure output dir exists (relative to this script)
        out_dir = os.path.join(os.path.dirname(__file__), "..", "output")
        os.makedirs(out_dir, exist_ok=True)

        ts = str(int(time.time()))
        df.to_csv(os.path.join(out_dir, f"main_obs_plot_{ts}.csv"), index=False)
        print("Results saved.")

        # Save compact NPZ for downstream plotting
        npz_payload = {
            "time": np.array(times, dtype=np.float32),
            "tar_dist": np.array(tar_dists, dtype=np.float32),
            "height_diff": np.array(height_diffs, dtype=np.float32),
            "stepsize": np.float32(stepsize),
        }
        np.savez(os.path.join(out_dir, f"main_obs_plot_{ts}.npz"), **npz_payload)
        # Also write/update a stable filename for quick plotting
        np.savez(os.path.join(out_dir, "main_obs_plot_latest.npz"), **npz_payload)
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(out_dir, 'gamma_plot_g10.png'))
        plt.show()

        # # 绘制到目标点的距离随时间变化的图表
        # plt.figure(figsize=(8, 4))
        # plt.plot(times, tar_dists, label="Distance to Target", color="green")

        # # 添加碰撞线
        # collision_threshold = 0.0
        # plt.axhline(y=collision_threshold, color="red", linestyle="--", label="Collision")
        ...
        # plt.grid(True)
        # plt.tight_layout()
        # plt.savefig('../output/distance_to_target_plot.png')
        # plt.show()

        # 绘制到目标点高度差随时间变化的图表（与仿真步同步记录）
        plt.figure(figsize=(12, 4))  # 调整图表宽度以拉长横轴
        # 使用 stepsize 与步数重建时间轴，避免时间采样抖动
        time_axis = np.arange(len(height_diffs), dtype=np.float32) * stepsize
        plt.plot(time_axis, height_diffs, label="Height Difference to Target", color="blue")
        ...
        # 设置图表标题和标签
        plt.title("Height Difference to Target Over Simulation Time")
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, 'height_difference_to_target_plot_wide.png'))
        plt.show()
