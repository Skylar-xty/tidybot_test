...
from rdf_base import query_sdf_batch_base
...
gamma_threshold = 10.0  # initial value, will be updated after SCA
...
def _pick_on_plane(mouse_x, mouse_y, plane_id):
    width, height, view_mat, proj_mat, *_ = p.getDebugVisualizerCamera()
    ray_from, ray_to = _compute_ray_from_mouse(mouse_x, mouse_y, width, height, view_mat, proj_mat)
    hits = p.rayTest(ray_from, ray_to)
    if hits and hits[0][0] == plane_id and hits[0][2] < 1.0:  # valid hit
        return np.array(hits[0][3], dtype=np.float32)
    return None

duration, stepsize = 26.0, 8e-3
if __name__ == "__main__":
    ...
    q_min_hardware = np.array(
        [-50, -50, -2.96706, -2.0944, -2.96706, -2.0944, -2.96706, -2.0944, -3.05433],
        dtype=np.float32,
    )
    ...
    qd_lim = np.array(
        [5, 5, 1.4835, 1.4835, 1.7453, 1.3090, 2.2689, 2.3562, 2.3562],
        dtype=np.float32,
    )

    acc_max = np.array([10, 10, 10, 10, 10, 10, 10, 10, 10], dtype=np.float32)

    robot = Panda(stepsize)
    robot.setControlMode("torque")
    p.setCollisionFilterPair(robot.robot, robot.robot, 9, 7, enableCollision=0)
    ...
    for i in range(int(duration / stepsize)):
        ...
        for ev in p.getMouseEvents():
            # ev: (eventType, x, y, buttonIndex, buttonState)
        ...
        # q = np.array(q)
        # qd = np.array(qd)
        qe = functions.compute_qe_base(q, qd)        # "emergency" stop q

        # ----------------------------------------
        # 5) SDF query for all 3 obstacles, q & qe
        # ----------------------------------------
        if esc_debug:
            theta_np = np.stack([q, qe], axis=0).astype(np.float32)  # (B=2,9)
            points_np = np.stack([
            ...
            dsts, grad_qs = query_sdf_batch_base(points_np, pose_np, theta_np)  # (2,3), (2,3,7)
        ...
        # ----------------------------------------
        Gamma, grad_gamma = functions.compute_gamma_and_grad(q[2:], qd[2:], threshold=gamma_threshold)
        ...
        tau_id = robot.solveInverseDynamics(q, qd, [0.0] * 9)
        ...
        u = cp.Variable(9)  # torque decision variable
        ...
        # if False:
            ...
            # if grad_gamma.shape[0] == 7:
            #     grad_gamma = np.concatenate((np.zeros(2), grad_gamma))  
            grad_q  = np.concatenate((np.zeros(2), grad_gamma[:7]))
            grad_qd = np.concatenate((np.zeros(2), grad_gamma[7:]))
            scale = 1
            g_eff_sc = scale * (0.5 * grad_q * dt_sc**2 + grad_qd * dt_sc)
            # c_const_sc = grad_q.dot(qd[2:]) * dt_sc
