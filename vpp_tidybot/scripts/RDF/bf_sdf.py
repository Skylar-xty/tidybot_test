...
import glob
import trimesh
from RDF import utils as utils
import mesh_to_sdf
import skimage
...
class BPSDF():
    ...
    def train_bf_sdf(self,epoches=200):
        ...
        # Load the robot mesh files and the corresponding SDF data
        ...
        # Train the Bernstein Polynomial SDF model for each robot link
        for i,mf in enumerate(mesh_files):
            ...
            # Normalize to unit sphere with centre at centroid of bounding box
            ...
            # This data is generated from sample_sdf_points.py. It contains SDF values for points near the surface and random points within the bounding box.
            data = np.load(f'./data/sdf_points/voxel_128_{mesh_name}.npy',allow_pickle=True).item()
            
            # Data preparation for near points and the corresponding SDF values, and random points and the corresponding SDF values
            ...
            # I am not sure why the negative SDF values are converted to positive here.
            sdf_random_data[sdf_random_data <-1] = -sdf_random_data[sdf_random_data <-1]

            # Initialize the weights for the Bernstein Polynomial basis functions
            wb = torch.zeros(self.n_func**3).float().to(self.device)

            # What is B supposed to represent here? I think its the basis functions, not sure. 
            B = (torch.eye(self.n_func**3)/1e-4).float().to(self.device)
            # loss_list = []

            # Learning in iterations
            for iter in range(epoches):

                # Choose random samples from near points and random points
                ...
                # Choose random samples from random points
                ...
                # Combine near points and random points
                ...
                # Update the weights using a Kalman filter-like update rule
